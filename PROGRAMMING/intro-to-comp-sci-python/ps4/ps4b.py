...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for char in alph_lower:
                i = alph_lower.index(char)
                d = 26 - i
                if shift < d:
                    dict[char] = alph_lower[i+shift]
                else:
                    dict[char] = alph_lower[-d+shift]
        for char in alph_upper:
                i = alph_upper.index(char)
                d = 26 - i
                if shift < d:
                    dict[char] = alph_upper[i+shift]
                else:
                    dict[char] = alph_upper[-d+shift] 
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            ...
            m = 0
            list = decrypted_text.split()
            for i in list:
                if i in self.get_valid_words():
                    m += 1
            if m > max:
                max = m
                maxs = s
